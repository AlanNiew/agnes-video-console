...
- `workspace.js`（约 1600 行）巨型 IIFE + 全量 innerHTML 重渲染——**M4 专项已推进：B0（vite）/ B1（互调清零）/ B2（任务中心按文件拆）/ B3-1（渲染纯函数拆至 `ws-render.js`）/ B3-2（配音+声音广场拆至 `ws-tts.js`）/ B3-3（会话状态抽至 `ws-state.js` 的 `st`）/ B3-4（工具 `ws-util.js` + 视频提交 `ws-video.js`，重刷走 bus）已交付**；剩余 renderProject 内联绑定与步骤动作拆分（B3-5…）与 CSS 拆分/收尾（B4）按 `docs/FRONTEND_REFACTOR_PLAN.md` 待专项执行。
...
- 前端已全面 ESM：`common`/`compare`/`state`/`task-meta` 为基础模块，`app.js` 为装配层（按序拉入
  ...
  任务中心域的按文件拆分与局部更新见 `docs/FRONTEND_REFACTOR_PLAN.md`（B2 已交付；workspace 拆分进行中，B3-1~B3-4 已交付）。
