...
- **即梦视频实测结论（2026-09，standard 会员）—— 视频不要指望即梦**：**排队以「天」计且队列净增长**（43 小时仅前进 6210 位，`queue_length` 反从 540k 涨到 553k），**并发上限 = 1**（只要 1 个任务在排队，后续提交即报 `ExceedConcurrencyLimit`，**锁死整个账户的视频功能**）。对比同机实测：**Agnes 免费档视频 327–527 秒（5–9 分钟）稳定出片**，连续 12 个任务全部成功。
