...
- `workspace.js`（约 1780 行）巨型 IIFE + 全量 innerHTML 重渲染——**M4 专项已推进：B0（vite）/ B1（互调清零）/ B2（任务中心按文件拆）/ B3-1（渲染纯函数拆至 `ws-render.js`）/ B3-2（配音+声音广场拆至 `ws-tts.js`，动作后重刷走 bus `ws-project-changed`）已交付**；剩余按步骤的动作/事件拆分（B3-3…）与 CSS 拆分/收尾（B4）按 `docs/FRONTEND_REFACTOR_PLAN.md` 待专项执行。
...
- 前端已全面 ESM：`common`/`compare`/`state`/`task-meta` 为基础模块，`app.js` 为装配层（按序拉入
  ...
  任务中心域的按文件拆分与局部更新见 `docs/FRONTEND_REFACTOR_PLAN.md`（B2 已交付；workspace 按步骤拆分进行中，B3-1/B3-2 已交付）。
