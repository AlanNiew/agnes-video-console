...
- **图片主力档从"最便宜的旧档 3.1"改为"性价比最优的 4.7"**（2k = **1 积分**、4k = **2 积分**）。
  起因：用户发现即梦网页端已有更多档位，而我们钉在最便宜的 3.1 上。核对后确认
  **CLI 自身默认档是 5.0**（`text2image -h` 明写 `default model_version: 5.0`），
  且从用户本机历史任务库 `~/.dreamina_cli/tasks.db` 挖出真实单价（零成本）：
  3.1@1k=1、4.6/4.7@2k=1、5.0@2k=3、5.0Pro@2k=8。
- **把选型规则写进 `core/constants.js` 注释**（4 条：同价取更新 / 一致性优先可换同价专档 /
  不要停在老代际 / 关键美术优先 4k 而非更高代际的 2k），避免以后凭"最便宜"直觉反复摇摆：
  · 主力 `4.7`（同价更新）；人像一致性专档 `4.6`（同价 1 分，官方称一致性更好，一行可切）；
  · 高画质 `5.0`（3 分，实测已被 4.7@4k=2 分支配）；最强 `5.0pro`（8 分）；
  · 老代际 `3.1`（1 分/1k，仅在明确需要 1k 时用）。
- 单价表多档由 estimated 升为 **measured**（网页端显示价 + 用户逐档核对）：
  `4.6/4.7 @2k=1、@4k=2`、`5.0 @2k=3`；并记录"4.7@4k(2 分) 同时比 5.0@2k(3 分) 更便宜且更清晰"。

### Added

- `tools/dreamina-model-drift.js`：**即梦模型清单漂移检测** —— 现场执行 `dreamina <子命令> -h`
  解析官方公开支持集，与白名单对账（missing/extra）并打印已知单价，漂移时退出码 1。
  已入服务器周任务（周日 05:40）。**只读、不提交、不花积分**。
  （首次运行即暴露自身 bug：视频档字段是 `modelVersion` 而工具只读了 `model_version` → 已修。）

### Fixed

- **换主力模型就会踩的坑**：`workers/auto.js` 的角色图路径硬编码 `size:'1k'`，
  而 4.6/4.7/5.0 只支持 2k/4k → 会被 `buildDreaminaImagePayload` 以"分辨率须为 2k / 4k"400 拒掉。
  改为**从模型 specs 取首个合法档位**（成本预估与 payload 构建两处）。
- 删掉重复的 `jimeng-image-4.7` 定义（eslint `no-dupe-keys` 抓到；同时导致价格查表取不到该档）。

### 说明（能力边界）

- 网页端的 **Seedream 5.0 Flash / 5.0 Lite / 图片美学模型 V8.2**（5.0 Lite/Flash @2k = 3 分）
  **CLI 不支持**：官方 help 的公开支持集只有 `3.0–5.0Pro`，CLI 升级到最新版（2026-09-10）仍未包含。
  要用它们只能在即梦网页端手工出图，本系统走不通（不是配置问题）；网页出好的图可导入本系统作参考图。
...
- **竖屏独立合成（方案 A）+ 净版成片**：渲染一次产出三件——
  `成片-N.mp4`（16:9 含字幕）、`成片-净版-N.mp4`（同画面**不烧字幕**，供人工改字幕/二次剪辑备用）、
  `成片-竖屏-N.mp4`（9:16：16:9 画面完整居中 + 同画面放大模糊作背景 + **字幕烧在底部安全区 15%**）。
  发布包 `抖音/成片-竖屏.mp4` 直接采用该竖屏产物（零重编码）。
  渲染参数新增 `emit_clean` / `emit_portrait`（默认均开，可关）。

### Fixed

- **竖屏字幕位置与重影**：此前竖屏切片拿「已烧字幕的成片」做模糊填充——字幕落在画面条带下缘，
  模糊背景里还有字幕重影，观感差。现竖屏以**净版**为输入、字幕另烧底部安全区，两个问题同时消除。
- **ffmpeg 多输出参数错位**：一次 filtergraph 双输出（成片 + 净版）时，输出选项必须与各自文件名
  成组出现（否则第二个输出拿不到 `-map`/编码参数）；同一音频标签不能映射到两个输出，需 `asplit`
  一分为二。
...
- **发布包 `抖音/封面-竖屏.png` 是坏文件**（`workers/render.js` 的 `fillAspect` 图片分支沿用
  `-c:v libx264` + `.png` 后缀）：ffmpeg 按后缀选 image2 封装却用 H.264 编码，产出
  「扩展名 .png、内容是裸 H.264 流」的文件（文件头 `00 00 00 01 67`），浏览器与平台均无法打开。
  **实测《幻灯屋》S1E05/E06 与 OIL-01 的竖屏封面全部中招**（已逐一重建为合法 PNG）；
  修复为图片分支显式 `-c:v png`，视频分支（竖屏切片）保持 libx264 不变。
- **回归护栏**：`test/mock-e2e.js` 增加**文件头魔数**校验——此前 e2e 只比对宽高，而 ffprobe
  能从裸 H.264 流里照样读出 720×1280，因此该 bug 一直未被拦下；现断言 `B站/封面.png` 与
  `抖音/封面-竖屏.png` 必须以 PNG magic（`89 50 4E 47 0D 0A 1A 0A`）开头。
