...
├── db/                  # Data layer: kernel.js (connection/DDL/migrations/tx) · sql.js (prepared-statement registry) · repos/ (settings/tasks/projects/renders table-family repos) · index.js (composite entry; require('./db') resolves here)
├── instance-lock.js     # Single-instance worker lock (atomic CAS on settings key, cross-process mutex; split from the data layer in M3)
