'use strict';
/**
 * clients/netmusic.js —— 音乐接口客户端（v1.4 BGM；M4 治理：去掉对 db 数据层的直接耦合）
 * 对接自托管的音乐接口服务（网易云曲库）：
 *   GET /search?keyword=&limit=          搜索歌曲
 *   GET /player?id=&level=              获取播放地址（地址有时效性，使用前现取）
 * 依赖注入：createNetmusicClient(settings)——settings 为 db 的 settings 命名空间，
 * 由装配层（routes/workers）在接线处传入，本客户端不再 require db。
 * 配置键：music_api_base / music_api_token / music_level。
...
function createNetmusicClient(settings) {
  function baseUrl() {
    let u = String(settings.get('music_api_base') || '')
      .trim()
      .replace(/\/+$/, '');
  ...
  function token() {
    return String(settings.get('music_api_token') || '').trim();
  }
  ...
  async function playUrl(id, level) {
    const base = baseUrl();
    if (!base) throw new ApiError(400, '尚未配置音乐接口地址（设置 → music_api_base）');
    const cfg = String(settings.get('music_level') || '');
    const lv = LEVELS.includes(String(level)) ? String(level) : LEVELS.includes(cfg) ? cfg : 'exhigh';
  ...
  return { search, playUrl, downloadBGM, baseUrl };
}

module.exports = { createNetmusicClient, LEVELS };
