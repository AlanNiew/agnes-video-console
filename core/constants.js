 ...
 */
const DREAMINA_IMAGE_DEFAULT_MODEL = 'jimeng-image-4.7';
/**
 ...
 *
 * 主力 / 备用策略（**选型规则，v2.6.14 定稿** —— 以后按规则选，不要凭"最便宜"直觉）：
 *
 *   规则 1 **同价取更新**：网页端 2k 档 4.6 与 4.7 同为 **1 积分** → 取更新的 `4.7`
 *          （官方："画质全面优化，指令响应能力再次提升"）。
 *   规则 2 **一致性优先时可换同价专档**：4.6 官方明确"人像一致性保持更好"，
 *          与 4.7 同价（1 积分）→ 若实测 4.7 在角色一致性上不如 4.6，一行切回即可。
 *   规则 3 **不要停在老代际**：`3.1`（1 积分/1k）已被 `4.6/4.7`（1 积分/**2k**）同价支配，
 *          仅在明确需要 1k 时使用。
 *   规则 4 **关键美术优先 4k 而非更高代际的 2k**：实测 `4.7@4k = 2 分`，
 *          同时比 `5.0@2k = 3 分` **更便宜且分辨率更高** —— 5.0 档在性价比上被 4.7 支配，
 *          故封面/主视觉首选 `4.7 @4k`；`5.0pro`（8 分）仅在明确需要其代际画风时上。
 *
 * 完整价格阶梯（网页端核对，2026-09-24）：
 *   4.6/4.7 → 2k = 1 分、4k = 2 分 ｜ 5.0 → 2k = 3 分 ｜ 5.0Pro → 2k = 8 分 ｜ 3.1 → 1k = 1 分
 *
 ...
 *   模型清单漂移用 `tools/dreamina-model-drift.js` 定期核对（已入周任务 05:40）。
 */
const DREAMINA_IMAGE_RATIOS = ['21:9', '16:9', '3:2', '4:3', '1:1', '3:4', '2:3', '9:16'];
const DREAMINA_IMAGE_MODELS = {
  // —— 主力：**同价取更新**（4.7 与 4.6 都是 2k = 1 积分，4.7 官方称"画质全面优化 + 指令响应再次提升"）——
  'jimeng-image-4.7': {
    provider: 'dreamina',
    subcommand: 'text2image',
    model_version: '4.7',
    resolutions: ['2k', '4k'],
    label: '即梦图片 4.7（2k/4k · 主力 · 同价更新 + 2k 仅 1 积分）',
  },
  // —— 人像一致性专档：与 4.7 同价（1 积分/2k），官方明确"人像一致性保持更好" ——
  'jimeng-image-4.6': {
    provider: 'dreamina',
    subcommand: 'text2image',
    model_version: '4.6',
    resolutions: ['2k', '4k'],
    label: '即梦图片 4.6（2k/4k · 人像一致性专档 · 同价 1 积分）',
  },
  ...
  // —— 省钱档：老代际 1k（4.6/4.7@2k 同价更清晰，故仅在需要 1k 时用） ——
  'jimeng-image-3.1': {
    provider: 'dreamina',
    subcommand: 'text2image',
    model_version: '3.1',
  ...
  'jimeng-image-5.0pro': {
    ...
    label: '即梦图片 5.0 Pro（1.5k/2k/4k · 最强 · 实测 2k=8 积分）',
  },
};
  ...
  image: {
    ...
    'jimeng-image-4.6': {
      perRequest: {
        ...
        // 且官方描述「人像一致性保持更好，性价比更高」→ 人像一致性专档。
        '2k': { points: 1, source: 'measured' },
        // 4k 亦已核对：**2 积分**
        '4k': { points: 2, source: 'measured' },
      },
    },
    'jimeng-image-4.7': {
      perRequest: {
        // 网页端显示价 + 用户核对（2026-09-24）：4.7 @2k 与 4.6 同为 **1 积分** → 同价取更新
        '2k': { points: 1, source: 'measured' },
        // 4k 亦已核对：**2 积分**（原按"宁高不低"估 6，高估 3 倍）
        '4k': { points: 2, source: 'measured' },
      },
    },
    'jimeng-image-5.0': {
      perRequest: {
        // 档位 image_basic_v5_2k / v50 系列：本机历史任务实测 text2image 5.0@2k = 3 积分、
        // image2image 5.0@2k = 3 积分（v2.6.12 从 estimated 升级为 measured）
        '2k': { points: 3, source: 'measured' },
        '4k': { points: 6, source: 'estimated' },
      },
    },
    'jimeng-image-5.0pro': {
