...
**待用户决策的四件事**（都影响 24h 产出的稳定性或质量）：

1. **默认模型**：`agnes-video-2.5-flash` 免费档排队严重（实测 503），无人值守时会连续浪费 ~22 分钟/条。
   建议把默认模型切到 **`agnes-video-v2.0`**（实测十秒接单、90 秒出片）；代价是画风/质量与 flash 不同，
   属创作决策。切换命令：`PUT /api/settings {"model":"agnes-video-v2.0"}`（或页面「设置」里改）。
2. **配音（旁白）**：服务器到 `api.fish.audio` 不通，当前线上成片**无旁白**。三条路：
   ① 给服务器一条可用出口（`FISH_PROXY` + 回填 `fish_api_key`）；② 本机生成配音后把 mp3 与绑定关系
   同步到线上（需要一个注入脚本）；③ 接受纯字幕成片。
3. **成片异地备份**：现有每日备份只覆盖**数据库**，`works/`（成片）没有异地副本。可选：本机定时拉取、
   或 rsync 到你另一台服务器。
4. **白名单健壮性**：安全组按源 IP 放行 —— 若你出口 IP 变动（家宽重播/手机流量），网页端会连不上
   （SSH 隧道仍可用，前提是 SSH 那条规则也允许）。建议给 SSH 留一条较宽的规则作为兜底入口。
