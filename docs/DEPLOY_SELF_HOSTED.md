...
> **不想 push 也可以**（兜底，非首选）：`git bundle create ..\.scratch\agnes-main.bundle main` 后 scp 过去，
> 在服务器 `git clone ~/ai-video/incoming/agnes-main.bundle ~/ai-video/app` 并 `git remote set-url origin <仓库地址>`。
> **推荐直接 push**：服务器已配好 GitHub 代理，`git pull` 实测 0.9 s（见第 8 步）。
...
> **链路问题已解决（2026-09-23）**：服务器 → `github.com:443` 直连**时好时坏**（实测过同一次执行里
> `git fetch` 成功、紧接着 `git pull` 135 s 超时）。现在机器上有了本地出口（第 9 节 mihomo），
> 给 git 配上**只对 GitHub 生效**的代理即可稳定拉取：
>
> ```bash
> git config --global http.https://github.com.proxy http://127.0.0.1:7890
> cd ~/ai-video/app && time git fetch --prune origin     # 实测 0.92 s（此前常超时）
> ```
>
> `deploy/bootstrap-server.sh` 会**自动探测** `127.0.0.1:7890` 并给 clone/pull 带上该代理，同时写进仓库级
> 配置（代理撤掉时 `git config --unset http.https://github.com.proxy` 即可）。
> 代理不可用时仍降级为告警继续，首次 clone 失败则给出下面的 bundle 兜底：
>
> ```bash
> # 本机
> cd D:\Programing\AI_Video_Create\agnes-video-console
...
```bash
# 更新代码（首选：直接拉，服务器已配 GitHub 代理）
cd ~/ai-video/app && git pull --ff-only && npm ci --no-audit --no-fund && npm run build
systemctl --user restart agnes-console
# 或者直接重跑引导（幂等）：bash ~/ai-video/app/deploy/bootstrap-server.sh

# 若哪天代理不可用、git pull 超时：改走 bundle 快进，无需 GitHub（兜底）
...
# 代理本身：systemctl --user status mihomo ；失效时 bash ~/ai-video/proxy/refresh-sub.sh
...
| **服务器→GitHub 链路不稳**（实测 135 s 超时） | clone/pull 失败，代码更新卡住            | **已解决**：给 git 配只对 GitHub 生效的代理（`http.https://github.com.proxy=http://127.0.0.1:7890`），实测 `fetch` **0.92 s**；引导脚本自动探测并带上该代理。代理不可用时降级为告警，仍可用 bundle 快进（第 8 步）                        |
...
| 环节                                | 结果                                                                                                                       | 证据                                                                                                                                                                                                                                                                                       |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 代码传输                            | 当时绕开 GitHub（服务器→github.com:443 两次 135 s 超时），走 git bundle 1.16 MB                                            | `~/ai-video/app`（bundle 快进至 `410fffc`），工作区干净。**2026-09-23 起改为代理直拉**（第 8/9 节）                                                                                                                                                                                        |
...
服务器→GitHub 直连时好时坏，曾实测 135 s 超时；**现在有本地出口，本脚本正是经代理下载**），
由 `refresh-geo.sh` 每周日 05:30 更新，
只有文件变化才重启 mihomo。
