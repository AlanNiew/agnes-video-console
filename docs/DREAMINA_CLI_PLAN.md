...
> **⚠️ v2.6.1 修正：单价必须按模型区分。** 实测同一分辨率下不同代际单价不同——
> `seedance2.0` 720p = **8 积分/秒**（E06 英雄镜头 10s **实扣 80 积分**），而 `seedance2.0fast` = 5 积分/秒，
> **相差 60%**；此前只按分辨率取单一值（`'720p': 5`）会**低报 60%**，而该数字正是弹给用户看的。
> 故价目表新增 `videoByModel` 覆盖档（优先级高于分辨率默认档），`estimateDreaminaCost` 先查它。
> 图片侧同理：`5.0pro @2k` 预估 6 分、**实扣 8 分**，故 `estimated` 档位的 UI 文案必须保留"以实际扣费为准"。
...
| 8   | **不可用/失败如何收场**（v2.6.1） | **自动改投免费档**（原地改 `model` + `request_json`），而非"保留排队等人工" | 人工不在场时旧行为等于把制作卡死；改投不新增状态与路由，Agnes 零成本兜底 |
| 9   | 单价表粒度（v2.6.1）              | 分辨率默认档 + **按模型覆盖档**                                             | 实测同分辨率差 60%，单值必估错                                           |
| 10  | 回退是否可关（v2.6.1）            | 设置项 `dreamina_fallback` 默认开                                           | 保留旧行为用于排查/回溯对比                                              |

---

## 11. 回退策略（v2.6.1 已落地）

**原则：会过期的即梦额度先用；一旦不可用就**立即**改投免费档，绝不把制作卡在等人工上。**

判定收口在一个**纯函数模块** `core/provider-policy.js`（零副作用、可单测）：

| 出口                                                                 | 作用                                                                                   |
| -------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `dreaminaUsable({installed, loggedIn, vipLevel, enabled})`           | 提交前可用性：`disabled` / `not-installed` / `not-logged-in` / `not-vip`               |
| `shouldFallbackFromDreamina(kind, {attempts, maxAttempts, enabled})` | 失败后是否回退：环境/合规/参数/业务错 → **立即**；`timeout`/`spawn-error` → 重试耗尽后 |
| `clampFreeSeconds` / `freeVideoSize`                                 | 回退映射的规格钳制（4–12s / 720P）                                                     |

「改投动作」由调用方执行（**只改 `tasks` 行，不新增状态与路由**）：

| 位置                      | 时机                                                                                                                              |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `workers/submitter.js`    | 即梦视频：环境未就绪 / `need-web-confirm` / 参数错 / CLI 业务错（含积分不足）/ 首帧取不到本地文件 / 重试耗尽 / 拿不到 `submit_id` |
| `workers/image-worker.js` | 即梦图片：提交阶段同上 + 查询阶段失败 / `gen_status=fail` / 成功但解析不到 URL                                                    |
| `workers/auto.js`         | 全自动成片角色图：**提交前预检**（未装/未登录/非 VIP/积分不足）直接走免费档，状态带 60s 缓存                                      |
| `services/payloads.js`    | `checkDreaminaGuard` 返回 `free_model` + `fallback{model,reason}`（三档语义不变）；`dreaminaToAgnes` 做任务映射                   |
| `public/task-meta.js`     | 额度不足不再硬拦，明确告知"继续将自动改用免费档"                                                                                  |

**回退映射规则**：提示词原样保留（不降级创意）；时长钳 4–12s、分辨率落 720P；首帧**仅在公网 http(s) URL** 时保留
（转 Agnes `keyframe` 模式），本地路径则降级纯文生并在任务 `error_message` 写明原因。
**没有 Agnes API Key** 或**提示词缺失**时无法回退，此时按原逻辑如实失败（不空转）。
