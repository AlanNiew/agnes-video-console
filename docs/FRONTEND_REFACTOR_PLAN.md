...
> 状态：**B0–B2 已交付、B3 进行中（B3-1~B3-6 已拆）**（B0 `9da51d8`；B1 系列；B2 `d6fdfed`；B3-1 `7d12f06`；B3-2 `3cf715b`；B3-3 `e215caf`；B3-4 `f3f50a3`；B3-5 `29e8510`；B3-6 `4b58ada`，2026-09）；实施前评审本文件。
...
| **M4-B3** 🔄（B3-1~B3-6 已交付） | 创作工作台逐步拆分（六步各自模块 + 自动成片时间线 + TTS 墙 + BGM/渲染），局部更新。B3-1：渲染纯函数/常量拆出 `ws-render.js`；B3-2：第⑤步配音+声音广场拆出 `ws-tts.js`（动作后重刷改经 bus `ws-project-changed`）；B3-3：会话状态抽 `ws-state.js`（`st`）；B3-4：通用工具 `ws-util.js` + 第④步视频提交 `ws-video.js`；B3-5：第③步角色图 `ws-char.js`；B3-6：第②步文案/分镜 `ws-story.js`；剩 renderProject 内联绑定（BGM/渲染面板）提取拆分与局部更新 | 功能等价 + 冒烟（build/e2e 已绿；浏览器人工冒烟待验）                                    | 高（2688 行闭包拆分，逐子系统人工核对） |
