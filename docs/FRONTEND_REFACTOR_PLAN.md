...
> 状态：**B0–B4 全部交付**（B0 `9da51d8`；B1 系列；B2 `d6fdfed`；B3-1 `7d12f06`；B3-2 `3cf715b`；B3-3 `e215caf`；B3-4 `f3f50a3`；B3-5 `29e8510`；B3-6 `4b58ada`；B3-7 `e22861a`；B4，2026-09）；实施前评审本文件。
...
| **M4-B1**（B1-1~B1-4 + 收尾 ✅） | 建 `state.js` 订阅模型 + 把 `common.js` 正式模块化；视图逐个从 `window.__*` 改为 import                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | app↔workspace 互调清零，`window.__app/__ws` 已删除（剩 `__ui/__audio` 共享组件 B2 处理） | 中                                      |
| **M4-B2** ✅（B2-0 + B2-1）      | 任务中心视图（原 app.js 1.4k 行）按文件拆（列表/看板/详情/弹窗），实现局部更新。B2-0 清 `window.__*`；B2-1 拆出 task-center/new-task/settings-panel/works-panel/task-meta，app.js 瘦身为装配层，沿用「集合签名」局部更新（lastColSig/lastListSig/lastPageSig/detailSig），顺带修复 inline `window.__app` 遗留                                                                                                                                                                                                                                                                                                                                                                                                   | 功能等价 + 冒烟（build/e2e 已绿，浏览器人工冒烟待验）                                    | 中（结构大拆，行为等价验证通过）        |
| **M4-B3** ✅（B3-1~B3-7 已交付） | 创作工作台逐步拆分（六步各自模块 + 自动成片时间线 + TTS 墙 + BGM/渲染），局部更新。B3-1：渲染纯函数/常量拆出 `ws-render.js`；B3-2：第⑤步配音+声音广场拆出 `ws-tts.js`（动作后重刷改经 bus `ws-project-changed`）；B3-3：会话状态抽 `ws-state.js`（`st`）；B3-4：通用工具 `ws-util.js` + 第④步视频提交 `ws-video.js`；B3-5：第③步角色图 `ws-char.js`；B3-6：第②步文案/分镜 `ws-story.js`；B3-7：第⑥步 BGM 面板拆出 `ws-bgm.js`、第⑦步成片渲染面板拆出 `ws-render-panel.js`（渲染按钮/风格预设/高级配置/渲染任务轮询），renderProject 内联绑定清空——渲染提交/轮询只改 `#wsRenderJobs` 子树（完成后不再整页重绘，保留面板已调配置），BGM 选用/清除只改 `#wsBgmCurrent`/`#wsPrecheck`/步骤⑥圆点（`refreshBgmArea`） | 功能等价 + 冒烟（build/e2e 已绿；浏览器人工冒烟待验）                                    | 高（2688 行闭包拆分，逐子系统人工核对） |
| **M4-B4** ✅ 已交付              | style.css 按视图拆分为 `public/styles/` 六文件（base / task-center / new-task / workspace / works / theme-light），逐行原样搬运；`index.html` 按序引用，vite 构建合并单产物；层叠安全经脚本校验（387 规则 0 跨文件顺序翻转）；`window.__*` 代码引用已为零（B2-0 完成时清零，B4 复核仅剩历史注释）；旧 `style.css` 删除。可选 playwright smoke 未引入（维持零新增依赖）                                                                                                                                                                                                                                                                                                                                          | 全绿                                                                                     | 低                                      |
