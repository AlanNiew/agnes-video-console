...
⚠️ 版本判定：`agnes-video-v2.0` **官方仍在售且免费**，本版界面不主推（保留兼容层，可经 `/api/tasks` 直接调用），界面统一使用
...
- **主推最新免费模型**；界面收敛：`agnes-2.0` / `image-2.0` 等官方已废弃模型不再接入；`agnes-video-v2.0` 官方在售免费，以「兼容保留、UI 默认不展示、API 可调用」形式保留；付费 `agnes-video-2.5` 界面上收敛隐藏，下一版统一模型注册表再开放；
...
5. 任务中心新建表单：加「✨ AI 优化提示词」按钮；模型选择收敛（默认 `agnes-video-2.5-flash`，付费 2.5 移入「高级」折叠，**v2.0 界面收敛不展示、API 兼容保留**）。
...
- 官方已废弃模型（agnes-2.0、image-2.0）的兼容层与界面入口；
- agnes-video-v2.0 的界面主推展示（官方在售免费，保留 API 兼容层，可经 `/api/tasks` 直接调用）；
- 本地图片公开外发（内网穿透等）方案。
