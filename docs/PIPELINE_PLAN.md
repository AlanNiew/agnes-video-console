...
| 视频生成 | `agnes-video-v2.0` | `POST /v1/videos` + `GET /agnesapi` | 异步轮询（复用现有 poller） | 免费 |

原则（按用户要求）：
- **只用最新模型**，旧模型（agnes-2.0、image-2.0、video-2.5 系列）本版不接入；
- **付费模型**（agnes-video-2.5）本版不重点做，界面上收敛隐藏，下一版统一模型注册表再开放；
- 解决「V2.0 切换麻烦」：流水线模式下模型**自动确定**，用户不再手动挑模型。
...
### 2.2 角色一致性的实现路径
- 角色立绘 → 视频用 **`image`（图生视频）模式**：一张定稿角色图 + 分镜提示词；
- 多角色/多场景 → 后续 M2 用 **`keyframes` 模式**（多图过渡）与**多图合成**（image 模型先合成再图生视频）；
...
POST /api/projects/:id/videos   从项目发起视频任务：
                                自动组装 {角色定稿图URL + 选定提示词 + 参数}
                                → v2.0 image 模式 → 复用 submitTask 入队
```
...
4. 第 4 步：展示「引用关系」（选定角色图缩略图 + 分镜提示词 + 时长/帧数参数）→「提交视频任务」→ 成功后链接跳转任务中心对应卡片；
5. 任务中心新建表单：加「✨ AI 优化提示词」按钮；模型选择收敛（付费 2.5 视频移入「高级」折叠）。
...
| 角色一致性有限（图生视频仍可能漂移） | 提示词显式约束 + 角色图构图简单正面 + M2 引入 keyframes/多图合成增强 |

## 8. 不做（本版明确排除）

- 付费模型（agnes-video-2.5 / 960P / 2K）深度接入；
- 旧模型（2.0 文本/image-2.0/video 2.5 系列）兼容层；
- 本地图片公开外发（内网穿透等）方案。
