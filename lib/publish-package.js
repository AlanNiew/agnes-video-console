...
const PLATFORMS = [
  {
    ...
    label: '哔哩哔哩',
    files: [
      { name: '成片.mp4', role: '视频（本项目原始画幅，直接上传）' },
  ...
  },
  {
    ...
    label: '抖音 / 快手',
    files: [
      { name: '成片-竖屏.mp4', role: '视频（9:16 竖屏，模糊背景填充，构图不裁切）' },
...
function buildPackageReadme({ project = {}, job = {}, copy = {}, notes = [] } = {}) {
  const files = PLATFORMS.map(
    (p) => `- \`${p.dir}/\`（${p.label}）\n` + p.files.map((f) => `  - \`${f.name}\` —— ${f.role}`).join('\n'),
  ).join('\n');
  return [
    ...
    '2. 视频选 `抖音/成片-竖屏.mp4`（9:16 竖屏，模糊背景填充，原构图完整）；封面选 `抖音/封面-竖屏.png`。',
